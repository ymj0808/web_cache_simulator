...
void virtual_node::SetIP(std::string ip) {
  this->ip = ip;
  // std::cout << "[SetIP = ]]\t" << this->ip << std::endl; // 10122020 Peixuan
  // vnode debug
}
...
void virtual_node::SetUid(int uid) {
  this->uid = uid;
  // std::cout << "[SetUID = ]]\t" << this->uid << std::endl; // 10122020
  // Peixuan vnode debug
}
...
real_node::~real_node() { this->virtual_node_hash_list.clear(); }
