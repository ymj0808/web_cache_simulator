...
public:
    ...
    bool operator<(const vnode& v) {
        return vnode_hash < v.vnode_hash;
    }
    bool operator==(const vnode& v) {
...
int find_nearest_node(unsigned int hash_value, vector<vnode>& list)
...
void trace_process(string in_path, string ref_path, string out_path, int rnode_num, int vnode_num, int from_rnode, int to_rnode, double ratio) {
    vector<rnode> rnodes;
    int ip_seg_3 = 0;
    int ip_seg_4 = 136;
    int v_id = 0;
    ...
    for (int i = 0; i < rnode_num; i++) {
        string tmp_ip_r = "192.168.0" + to_string(ip_seg_3) + "." + to_string(ip_seg_4);
...
int main(int argc, char* argv[]) {
    ...
    trace_process(in_path, ref_path, out_path, rnode_num, vnode_num, from_node, to_node, ratio);
}
