...
*/
FilterCache::FilterCache() : LRUCache(), _nParam(2) {}
...
*/
ThLRUCache::ThLRUCache() : LRUCache(), _sizeThreshold(524288) {}
...
*/
ExpLRUCache::ExpLRUCache() : LRUCache(), _cParam(262144) {}
...
AdaptSizeCache::AdaptSizeCache()
    : LRUCache(), _cParam(1 << 15), statSize(0), _maxIterations(15),
...
bool AdaptSizeCache::lookup(SimpleRequest *req) {
  ...
  /**
      ...
      if(_longTermMetadata.count(id)>0 && _longTermMetadata[id].size !=
     req.size()) {
...
void AdaptSizeCache::reconfigure() {
  ...
  for (auto it = _longTermMetadata.begin(); it != _longTermMetadata.end();
       it++) {
    it->second.requestCount *= EWMA_DECAY;
  }

  // persist intervalinfo in _longTermMetadata
  for (auto it = _intervalMetadata.begin(); it != _intervalMetadata.end();
       it++) {
    auto ewmaIt = _longTermMetadata.find(it->first);
    if (ewmaIt != _longTermMetadata.end()) {
      ewmaIt->second.requestCount +=
          (1. - EWMA_DECAY) * it->second.requestCount;
...
double AdaptSizeCache::modelHitRate(double log2c) {
  ...
  for (size_t i = 0; i < _alignedReqCount.size(); i++) {
    sum_val += _alignedReqCount[i] *
               (exp(-_alignedObjSize[i] / pow(2, thparam))) *
               _alignedObjSize[i];
  }
  if (sum_val <= 0) {
    return (0);
...
void S4LRUCache::admit(SimpleRequest *req) { segments[0].admit(req); }
...
void S4LRUCache::evict() { segments[0].evict(); }