...
void GreedyDualBase::evict() {
  ...
  if (_valueMap.size() > 0) {
    ...
    if (lit == _valueMap.end()) {
      std::cerr << "underun: " << _currentSize << ' ' << _cacheSize
                << std::endl;
    }
...
long double GDSFCache::ageValue(SimpleRequest *req) {
  CacheObject obj(req);
  return _currentL +
         static_cast<double>(_reqsMap[obj]) / static_cast<double>(obj.size);
}

/*
  LRU-K policy
*/
LRUKCache::LRUKCache() : GreedyDualBase(), _tk(2), _curTime(0) {}
...
long double LRUKCache::ageValue(SimpleRequest *req) {
  ...
  // std::cerr << id << " " << _curTime << " " << _refsMap[id].size() << " " <<
  // newVal << " " << _currentL << std::endl;
  return newVal;
}
